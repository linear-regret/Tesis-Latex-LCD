...
\subsection{Energía S} \label{sec:S-Energy}
...
\begin{equation} \label{eq:S_energy}
    E_s(A)=\sum_{i\neq j} ||a_i-a_j||^{-s},
\end{equation}
mientras $s\rightarrow \infty$ la energía de Riesz tiende a soluciones más uniformes. Además, el parámetro $s$ es independiente de la forma y ha sido usado para producir conjuntos uniformemente distribuidos en una clase grande de variedades $d$-dimensionales rectificables.
...
\subsection{Wilcoxon} \label{sec:Wilcoxon}
...
Las suposiciones de la prueba de Wilcoxon \cite{hollanderNonparametricStatisticalMethods2015} son parecidas a aquellas de la de Kruskal Wallis:

\begin{itemize}
    \item Las observaciones pareadas tienen un carácter ordinal o real y los $Z_i$ son mutuamente independientes. 
    \item Cada $Z$ viene de una distribución continua que es simétrica alrededor de una mediana común $\theta$, llamada el efecto del tratamiento. Si $F_i$ es la función de probabilida de $Z_i$, tenemos que 
...
\begin{itemize}
    ...
    \item Se ordenan las diferencias absolutas \(|d_i|\) y se les asigna (al igual que en Kruskal-Wallis) rangos \(R_i\) desde el más pequeño al más grande. Si hay empates (diferencias absolutas idénticas), se asigna el rango medio.
...
\begin{table}[H]
    \centering
    \begin{tabular}{|c|c|c|c|}
    \textbf{B} & \textbf{$(r_1,\ldots,r_B)$} & \textbf{Probabilidad de que ocurra dado $H_0$} & \textbf{T\textasciicircum{}+=\textbackslash{}sum\_\{i=1\}\textasciicircum{}B r\_i} \\ \hline
    0          &                             & $\frac{1}{8}$                                  & 0                                                                                  \\
    1          & $r_1=1$                     & $\frac{1}{8}$                                  & 1                                                                                  \\
    1          & $r_1=2$                     & $\frac{1}{8}$                                  & 2                                                                                  \\
    1          & $r_1=3$                     & $\frac{1}{8}$                                  & 3                                                                                  \\
